...
you — ready to load through the product's normal import flow. It bundles five
...
- **Manage your portfolios** — connect to your own Portfolio installation and work
  ...
  picked, copy a scenario in one call and vary the copy — swapping the objective metric
  or flipping maximize/minimize, so your baseline stays put and you can compare the two
  — or copy a portfolio's calculation model into another portfolio.
  It remembers which portfolio and scenario you're working on, so you don't repeat
  yourself.
...
- **Your Portfolio installation** — needed only for the *Manage your portfolios*
  ...
  Run **`/qp-hi`** — it checks the connection and, if it isn't set up, offers to
  walk you through it (your key never gets pasted into the chat). In short: create an
  **MCP API key** under **Account Settings** in
  Portfolio — it's shown only once — then add it to your own Claude Code config and
  restart:
  ...
  `/qp-hi` covers the rest (config file layout, local installs, troubleshooting).
