#!/usr/bin/env python3
"""Refresh the bundled portfolio.schema.json from the committed server copy.
...
BUNDLED = SKILL_DIR / "schemas" / "portfolio.schema.json"
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Sync the bundled portfolio schema.")
    ap.add_argument("--server-root", help="path to a cp-portfolio checkout root")
    args = ap.parse_args(argv)

    source = find_server_schema(args.server_root)
    if source is None:
        print("ERROR: could not locate server/Esi.Sp.Portable/Schemas/portfolio.schema.json.\n"
