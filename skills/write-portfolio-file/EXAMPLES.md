...
`scripts/validate_portfolio.py` (exit 0).
...
This file pairs with the `write-model-file` skill's example 6 (`Value` and
`Capex` inputs with `derived: ["Total"]`; a scenario maximizes `Value#Total`
under a `Capex#Total` limit). Because `Total` sums every period, the one-time
amounts are **zero-padded series** (`[10, 0, 0]`) — a flat `[10, 10, 10]` would
silently make the item cost 30 against the budget. Nothing validates that the
model and data file agree; keep names, units, and this data shape in sync.
    ...
    "opportunity_outcomes": [
      { "opportunity_name": "Candy", "outcomes": [
        { "outcome_name": "Base", "metric_values": [
          { "metric_name": "Value", "values": [20, 0, 0], "unit": "$ MM" },
          { "metric_name": "Capex", "values": [10, 0, 0], "unit": "$ MM" } ] } ] },
      { "opportunity_name": "Chocolate", "outcomes": [
        { "outcome_name": "Base", "metric_values": [
          { "metric_name": "Value", "values": [40, 0, 0], "unit": "$ MM" },
          { "metric_name": "Capex", "values": [30, 0, 0], "unit": "$ MM" } ] } ] }
    ]
  },
  "selection_constraints": {
    "opportunity_limits": [
