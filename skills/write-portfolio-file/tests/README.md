...
- `broken_semantic.json` — **schema-valid but semantically broken**: bogus
  outcome-dependency outcome names, `total_maximum < total_minimum`, group
  member not in `input_data`. Proves the validator catches the
  cross-field/cross-reference rules JSON Schema cannot express → exit 1,
  `schema_errors == 0`, `hard_errors >= 4`. (Uniqueness and single-field ranges
  are now enforced by the schema, so this fixture carries none of those. It
  also carries a self-dependency and a `time_period_maxima < minima` entry,
  but both are only WARN — the latter is accepted on import and surfaces as a
  selection constraint conflict.)
...
Invoke the skill itself (e.g. `/write-portfolio-file ...`) and have it write
