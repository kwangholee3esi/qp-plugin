...
- **Opportunity** — an investment candidate the optimizer may select. Its
  **input data** is the time-series and scalar data (capex, opex, production,
  ...
  (`wiki/concepts/portfolio-model.md`; `wiki/expressions/data-considerations.md`).
- **Outcome** — a possible realisation of an opportunity. A **certain**
  ...
  (`wiki/expressions/data-considerations.md`). You may add as many outcomes to an
  opportunity as needed (`wiki/expressions/data-considerations.md`).
...
- `attributes.opportunity_attributes[].is_order_global` — omit it when authoring a
  new portfolio (defaults to false, meaning `order` positions the opportunity
  within its own block). Set it to `true` only to pin an opportunity to a manually
...
- `time_period_maxima` is indexed by planning period; periods past the end of
  the array are treated as `0` (forbidden), same as an explicit `0` entry — so
  give one positive entry per period the opportunity may actually be selected
  in, not just for however many periods you bothered to list.
...
`time_period_maxima[i] ≥ time_period_minima[i]` is only WARN — the server
accepts a maximum below the matching minimum on import; it surfaces there as a
selection constraint conflict rather than an import failure. (Single-field
non-negativity is schema-enforced via `minimum: 0`.) `interior_limit >
