...
## settings — omit it unless creating a portfolio or the user names a date/horizon/rate

`settings` carries the portfolio's name, its calculation calendar and its discounting
and inflation rates. **Leave it out by default** when importing into an existing
portfolio: when present it *replaces* the portfolio's settings, so a block written
from guesses overwrites the real numbers of a live portfolio.

Author it when `portfolio_import` is to CREATE the portfolio (no `portfolioId`) — then
`portfolio_name` is what names it, and its required fields set the calendar — or
when the user actually states a start date, a planning horizon, or a discount/inflation
rate. Then:

- **`portfolio_name` is optional — write it when this file is the one creating the
  portfolio, and leave it out otherwise.** It is not inert on a portfolio that already
  exists; `portfolio_import` describes what it does there. That is also why an export
  you are re-importing into a *different* portfolio needs it edited out first.
- **`time_unit` is the portfolio's periodicity.** A portfolio this file creates takes
  the file's unit. Changing an existing portfolio's unit needs the user's explicit
  say-so, so match the target's unit unless they asked for the change —
  `portfolio_import` refuses a mismatch and spells out the consequences to put to them.
...
- Importing into an existing portfolio, and no start date, planning horizon or
  discount/inflation rate mentioned → omit `settings` (see "settings — omit it
  unless…"). Never write it from defaults. When this file is the one *creating* the
  portfolio, `settings` is required — it carries the name and the calendar.
- **Selection limits unspecified → default each opportunity to `integer_only: false`,
  `total_minimum: 0`, `total_maximum: 1`, `time_period_minima: []`,
  `time_period_maxima: [1]`** (one `opportunity_limits` entry per opportunity). The `[1]` is a single-period
