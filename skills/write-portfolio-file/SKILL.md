...
the order **model → portfolio → scenario**. The metric/attribute names and
units in this data file must exactly match the companion model file — they are
the join key and no validator checks the join (mind data shapes too: a model
`Total()` sums every period, so one-time amounts are zero-padded series).
...
2. **Ground.** Use [REFERENCE.md](REFERENCE.md) for field meanings and domain
   ...
   query the QP knowledge vault (the `qpwiki` MCP server's `vault` tool) and cite the page.
...
3. **Author.** Build the JSON following [REFERENCE.md](REFERENCE.md) and the
   ...
   `opportunity_limits` entry per opportunity defaulting to `integer_only: false`,
   `total_minimum: 0`, `total_maximum: 1`, `time_period_minima: []`, and a
   **single-period selection horizon** `time_period_maxima: [1]`. The per-period maxima must be NON-empty:
   ...
   `selection_group`, `settings` ONLY when the description calls for them —
   write `settings` when this file is the one that creates the portfolio —
   `portfolio_name` is what names it — or when the user names a start date, a
   planning horizon or a discount/inflation rate (REFERENCE.md → "settings").
   Conventions:
   ...
   (name max-lengths, non-negative ranges, unique names). Field limits are stated
   in `schemas/portfolio.schema.json` and enforced by the validator — read them
...
5. **Write or surface.**
   - Exit `0` → write the final file — when the user gave no path, default to
     `<kebab-case-topic>-portfolio.json` in the working directory and ask only
     if that file already exists — and report any remaining warnings.
...
- `schemas/portfolio.schema.json` — the QPortfolio Portfolio schema the validator
  checks against.
- [REFERENCE.md](REFERENCE.md) — field meanings, the full check catalog, domain
  conventions (cited), and the bundled schema.
