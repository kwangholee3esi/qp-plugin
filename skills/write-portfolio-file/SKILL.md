...
3. **Author.** Build the JSON following [REFERENCE.md](REFERENCE.md) and the
   ...
   `settings` in particular replaces a portfolio's calendar and rates on import, so
   write it only when the user names a start date, a planning horizon or a
   discount/inflation rate (REFERENCE.md → "settings"). Conventions:
   ...
   (name max-lengths, non-negative ranges, unique names) are stated in
   `schemas/portfolio.schema.json` and enforced by the validator — read them there
