...
def discover_schema(explicit: str | None, target: Path) -> tuple[Path | None, str]:
    ...
    if override_dir:
        candidate = Path(override_dir) / "portfolio.schema.json"
...
# so jsonschema ignores it unless we register a handler. The product's importer
# enforces it the same way; we extend the validator so these land in the normal
# schema-error stream.
# --------------------------------------------------------------------------- #
...
def _limit_numeric_checks(limit, path, out, label):
    ...
    for idx in range(min(len(mins), len(maxs))):
        ...
        if isinstance(lo, (int, float)) and isinstance(hi, (int, float)) and hi < lo:
            out.append(finding(
                WARNING, "limit.period_max_lt_min", f"{path}.time_period_maxima[{idx}]",
