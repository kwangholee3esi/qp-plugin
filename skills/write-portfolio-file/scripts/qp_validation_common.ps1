...
function Invoke-SchemaNode($schema, $instance, $segs, $root, $errs) {
    ...
    foreach ($kw in $schema.PSObject.Properties) {
        ...
        switch -CaseSensitive ($name) {
            ...
            'minItems' {
                if ((Test-IsArray $instance) -and (@($instance).Count -lt $val)) {
                    Add-SchemaError $errs $segs 'minItems' "is too short"
                }
            }
            'uniqueKeys' {
                if ((Test-IsArray $instance) -and (Test-IsArray $val)) {
                    # Ordinal comparer: PS hashtables are case-insensitive by default,
                    # but Python/jsonschema key comparison is case-sensitive.
...
function Invoke-LimitNumericChecks($limit, $path, $out, $label) {
    ...
    for ($idx = 0; $idx -lt $shared; $idx++) {
        ...
        if ((Test-IsNumber $lo) -and (Test-IsNumber $hi) -and ($hi -lt $lo)) {
            [void]$out.Add((New-Finding $SevWarning 'limit.period_max_lt_min' "$path.time_period_maxima[$idx]" `
                        "${label}: time_period_maxima[$idx] ($(Format-Num $hi)) < time_period_minima[$idx] ($(Format-Num $lo))" `
                        $hi 'Accepted on import but surfaces as a selection constraint conflict.'))
        }
    }
...
# --------------------------------------------------------------------------- #
# Schema discovery (--schema, then QP_SERVER_SCHEMA_DIR, then the bundled copy).
# --------------------------------------------------------------------------- #
function Find-Schema($explicit, $targetPath, $scriptRoot, $schemaFileName) {
    if ($explicit) { return @{ path = $explicit; source = 'explicit' } }
    if ($env:QP_SERVER_SCHEMA_DIR) {
        $cand = Join-Path $env:QP_SERVER_SCHEMA_DIR $schemaFileName
        if (Test-Path -LiteralPath $cand -PathType Leaf) { return @{ path = $cand; source = 'server-in-repo' } }
    }
...
function Invoke-QpValidation($Argv, $SchemaFileName, $ScriptRoot, $SemanticFn, [bool]$SupportsPortfolio) {
    ...
    if ($disc.source -eq 'server-in-repo') {
        ...
        if ((Test-Path -LiteralPath $bundled -PathType Leaf) -and ((Get-Sha256 $bundled) -ne (Get-Sha256 $disc.path))) {
            $drift = 'bundled schema differs from the schema at QP_SERVER_SCHEMA_DIR'
        }
    }
