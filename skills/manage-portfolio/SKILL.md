...
description: >-
  ...
  clones a scenario to compare, changes what it optimizes for, edits a scenario
  in place (a metric limit, pinned selections, a rule override), and copies or
  varies a portfolio's calculation model. Use when the user refers to their OWN
  live Portfolio data, or names a portfolio or scenario — "list my portfolios",
  "what scenarios are in North Sea 2027", "load this file into Portfolio",
  "optimize the Base Case", "why is my scenario infeasible", "what did the
  optimizer pick", "cap capex at 50MM in Base Case", "drop that limit",
  "maximize NPV instead", "copy the model from North Sea into a new portfolio",
  "who am I connected as". NOT for how-does-Portfolio-work questions (use
  `qp-help`) nor for authoring a JSON file from a description (use the
  `write-*-file` skills, whose output this one loads).
---
...
2. **Load authored files in.** The `write-*-file` skills produce the JSON; the
   ...
   before uploading. `portfolio_import` starts a **background job** — follow it with
   flow 3 before doing anything else with that portfolio.
   ...
   - **Read before you write.** `portfolio_export` writes a portfolio's own data out
     ...
     purely additive; this is the same judgement flow 7 states for scenarios. Narrow
     `sections` to what you are changing — the blast radius of a re-import is per
     section, and the tool's own description spells out what each one does.
...
6. **Edit a scenario in place.** For a targeted change to an existing scenario — a
   metric limit, the objective, pinned selections, a rule override —
   `scenario_describe`, `scenario_read` and `scenario_patch` act on the scenario
   directly: no files, no buckets. Look first, then patch **echoing the paths the
   read answered with** — never construct one by hand. REFERENCE.md carries the ops
   cheat-sheet and the traps. In place only — to keep the baseline, use flow 7.

7. **Clone and vary.** `scenario_copy` takes the same `ops` as `scenario_patch`, so the
   copy and the change are one call. The duplicate carries more than an exported document
   does, so the baseline is safe by construction and nothing needs renaming. Then optimize
   both and report them side by side — objective values, not just which opportunities
   differ. Read the source first when the ops need real paths, and use flow 6 to patch the
   copy again later.

   Two fallbacks, both in REFERENCE.md: changing the **original** in place, and building a
   scenario from a **document** when there is no original to copy.

   To just *show* one part of a scenario's setup, `scenario_read` it — no file needed.

8. **Copy or vary a model.** `model_export` carries the recipe and the `section`
...
- **Always mark scratch work `[agent-tmp] `.** On `portfolio_create` that is
  `temporary: true`; on `portfolio_import` you write the prefix into
  `settings.portfolio_name` yourself. Skip it only when the user says it's a keeper.
- **A `curl` that cannot *connect* is not an expired link** — `ping` describes the two
  addresses and how to tell a routing problem from a configuration one. Hand the user the
  command to run.
