...
the order **model → portfolio → scenario** — the scenario comes last and only
references names the earlier files define (`--portfolio` verifies those
against the portfolio; objective/limit metrics may also be the model's
computed or derived names, e.g. `Value#Total`).
...
3. **Author.** Build the JSON following [REFERENCE.md](REFERENCE.md) and the
   ...
   - Conventions: snake_case keys, string enums, omit optional fields rather than
     ...
     keys). Field limits are stated in `schemas/scenario.schema.json` and
     enforced by the validator — read them there rather than from memory.
...
5. **Write or surface.**
   - Exit `0` → write the final file — when the user gave no path, default to
     `<kebab-case-topic>-scenario.json` in the working directory and ask only
...
- `schemas/scenario.schema.json` — the QPortfolio Scenario schema the validator
  checks against.
