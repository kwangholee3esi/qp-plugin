...
- `broken_semantic.json` — **schema-valid but semantically broken**: duplicate
  ...
  `total_maximum < total_minimum`, and a `group_limits` entry pointing at a
  nonexistent group. Proves the validator catches the cross-field/cross-reference
  rules JSON Schema cannot express → exit 1, `schema_errors == 0`,
  `hard_errors >= 3`. (Most uniqueness and single-field ranges are now enforced
  by the schema, so this fixture omits those. It also carries a
  `time_period_maxima < minima` entry, but that's only a WARN now — accepted on
  import, surfaces as a selection constraint conflict.)
...
Invoke the skill itself (e.g. `/write-scenario-file ...`) and have it write
