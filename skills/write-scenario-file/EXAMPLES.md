...
`scripts/validate_scenario.py` (exit 0).
...
The leanest meaningful scenario: a name and an optimization objective. No metric
limits, no rule overrides (so the portfolio's
...
**Maximum** metric limit; periods not listed are unconstrained.
