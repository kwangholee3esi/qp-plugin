...
# schema (last-'#' suffix match, case-insensitive).
DERIVED_TYPES = {
...
# max_args None = unbounded. Mirrors the QPortfolio expression language.
FUNCTIONS = {
    "getmetricvalue": (1, 2),
    "getdiscounted": (1, 2),
    "getinflated": (1, 2),
...
def discover_schema(explicit: str | None, target: Path) -> tuple[Path | None, str]:
    """Return (path, source). --schema wins, then QP_SERVER_SCHEMA_DIR, then bundled."""
    if explicit:
        return Path(explicit), "explicit"

    override_dir = os.environ.get("QP_SERVER_SCHEMA_DIR")
    if override_dir:
        candidate = Path(override_dir) / "model.schema.json"
        if candidate.is_file():
            return candidate, "server-in-repo"
...
# handler. The product's importer enforces it the same way; we extend the
# validator so these land in the normal schema-error stream.
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
def _norm_name(s):
    """Tabs/newlines/CRs become spaces, then trim (mirrors QPortfolio)."""
    return re.sub(r"[\t\r\n]", " ", s).strip()
