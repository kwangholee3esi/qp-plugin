#!/usr/bin/env python3
"""Refresh the bundled model.schema.json from the committed server copy.
...
BUNDLED = SKILL_DIR / "schemas" / "model.schema.json"
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Sync the bundled model schema.")
    ap.add_argument("--server-root", help="path to a cp-portfolio checkout root")
    args = ap.parse_args(argv)

    source = find_server_schema(args.server_root)
    if source is None:
        print("ERROR: could not locate server/Esi.Sp.Portable/Schemas/model.schema.json.\n"
