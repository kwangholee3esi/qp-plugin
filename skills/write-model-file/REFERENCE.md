...
(`wiki/concepts/metric.md`), expressions (`wiki/concepts/expression.md`,
`wiki/expressions/expressions-sheet-basics.md`), calculation level
(`wiki/expressions/expression-calculation-level.md`), filtering
(`wiki/expressions/filtering-expressions.md`), derived metrics
(`wiki/concepts/derived-metric.md`), attributes (`wiki/concepts/attribute.md`,
...
- **When to reach for a derived metric.** **Use one whenever the origin is linear
  ...
  **master-data** metrics. Non-linear signals: `^`, `If`/`Min`/`Max`/`Abs`,
  ...
  non-master-data metrics (`wiki/expressions/expressions-master-data.md`,
...
- `metric_type` → omit for inputs; state it for `MasterData`/`Computed`.
- `level` → `"Outcome"` for per-opportunity calculations unless the user
  ...
  (An authoring recommendation, not the schema's default — emitting it explicitly
  is what makes the difference moot.)
