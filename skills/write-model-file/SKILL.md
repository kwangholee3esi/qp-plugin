...
3. **Author.** Build the JSON following [REFERENCE.md](REFERENCE.md) and the
   ...
   - **A metric definition is complete.** On re-import into an existing model,
     any omitted optional property — unit, color, format, category, description
     and the rest (`wiki/import-export/export-and-import-a-model.md`) — CLEARS
     the stored value, so state every property the user wants kept. For a
   ...
   - Conventions: snake_case keys, string enums, omit optional fields rather
     than writing `null`, never emit `NaN`/`Infinity`. Field limits (name
     max-lengths, no braces in names, `#RRGGBB` colors) are stated in
     `schemas/model.schema.json` and enforced by the validator — read them there
...
- `schemas/model.schema.json` — the QPortfolio Model schema the validator
  checks against.
- [REFERENCE.md](REFERENCE.md) — the expression language, re-import semantics,
  the full check catalog, domain conventions (cited), and the bundled schema.
