...
- **Explain, don't operate.** Answering "how do I X" with the product's own steps is
  ...
  file in, optimize, read results, change what a scenario optimizes for), and
  `write-portfolio-file` / `write-scenario-file` / `write-model-file` build an
  import file from a description.
