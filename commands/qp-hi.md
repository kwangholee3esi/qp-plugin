...
- If both succeed → step 1b.
- If `qpmcp` isn't available at all, or either call fails → go to step 3.

Relay every `WARNING:` from `ping` in one line, in the user's words — there can be more
than one. Don't soften it; `ping` itself says what each one means.

## Step 1b — run ping's own shell check

`ping`'s description gives you the two URLs to curl, the commands to run, and how to read
the result. **This is the "once per session" it asks for** — do it here, and follow it
exactly. Two things it cannot know:

- **Both reachable → go straight to step 2 and say nothing about it.** A working
  connection is not news.
- **A `200` from `server=` is not a promise that transfers work.** It proves the host is
  up and correctly addressed, not that the upload and download routes are mounted on it.
  A transfer can still fail afterwards — that is what the failure table in the
  `manage-portfolio` skill is for.

If `server=` is genuinely unreachable, still go to step 2 and greet normally — read tools
work fine — but add one line telling the user plainly what is unavailable:

> One thing: the Portfolio server at `<server= URL>` isn't responding. Reading your data
> still works, but **file uploads, downloads and any job — imports and optimization runs —
> won't run until it's back.** A job I start would sit queued rather than fail, so I'll
> hold off on those.

Then hold to that: don't start imports or optimizations, and don't mint transfer links.
...
Use `ping`'s `mcp=` URL — that's the installation they're connected to, and the only
place you learn it, so don't guess it or reuse `$ARGUMENTS`. (`server=` is a different
address; never greet with it.) If `mcp=` reads `(unset)` or `ping` warned about it, greet
without a URL and relay the warning instead.
