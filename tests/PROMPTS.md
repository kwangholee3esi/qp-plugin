...
Invoke `/write-portfolio-file` with:
...
Invoke `/write-scenario-file` (companion portfolio = the file above) with:
...
## `model/model_full_coverage.json`

Invoke `/write-model-file` with:

> Build me a model (qp_version 4.5) for an oil portfolio.
>
> Inputs: **Oil Rate** (bbl/d, format `#,##0.0`, colour #2CA02C, category "Inputs", with a
> description) and a scalar **Working Interest** (fraction). Master data: **Oil Price**
> ($/bbl, category "Master Data").
>
> Computed metrics:
> - **Reserves** (Outcome level, MMbbl) — a declining balance: opening balance 1000 in the
>   first period, then previous Reserves minus Oil Rate.
> - **Revenue** (Outcome level, $ MM, scaled by interest) = Oil Rate × Oil Price / 1000,
>   except a 10% royalty haircut for USA opportunities — apply it to Well A only, in
>   periods 0–1, to every outcome except "Fail". Give Revenue **every** derived metric
>   (PT, Disc, Inf, CumSum, CumSumDisc, CumSumInf, Total, TotalDisc, TotalInf, MaxAT).
> - **Net Cashflow** (Opportunity level, $ MM, scaled by instance count, not hidden) =
>   Revenue − Oil Rate.
> - **Country Cashflow** (Group level grouped by Country, $ MM) = total Revenue.
> - **Scenario Tax** (Scenario level, $ MM, hidden) = 30% of cumulative Revenue to date.
>
> Attributes: **Country** (String, editable, not scenario-level; Canada #1F77B4 then USA
> #FF7F0E), **Royalty Tier** (Numeric, one characteristic 0.05), and **Start Date** (Date,
> scenario-level, one characteristic 2027-01-01).

Expected: validates clean (exit 0, 0 hard errors, 0 warnings).

---

## `manage-portfolio` — live flow check

This skill ships no validator scripts, so there is nothing to unit-test: it is
instructions over live MCP tools. Verify it by hand, once, against a running
`Esi.Sp.Mcp` host and a `qpmcp` entry in `~/.claude.json` (run `/qp-connect`). In a
fresh session:

> Who am I connected as?

Expect one line naming the effective user. Then:

> Load `tests/model/model_full_coverage.json` and
> `tests/portfolio/portfolio_full_coverage.json` into a new portfolio called
> "coverage check", then list its scenarios.

Expect: an upload, `create_portfolio(temporary: true)` — so the name comes back
prefixed `[agent-tmp] ` — a job id, polling to `succeeded`, then `list_scenarios`.
Then:

> Optimize it and tell me what it picked.

Expect: the target named before the run, polling at a sane cadence, then
`get_selections` with the `solver_status` reported. Then:

> Why did it come out that way?

Expect: solver status, objective vs best bound, the gap, solve time — and an explicit
statement that per-constraint binding can't be determined, because no tool returns
metric values per opportunity. Then:

> Copy that scenario but cap capex at 50 and optimize both so I can compare.

Expect: `export_scenario` → download to a temp dir → edit → validate → upload →
`import_scenario` producing a **new** scenario, with the original untouched. Finally:

> Copy that portfolio's model into a new empty portfolio called "model only".

Expect: `create_portfolio` → `export_model` → the token passed **straight** to
`import_portfolio_data` with no download and no upload, then a job polled to
`succeeded`. Then:

> Add a metric called Netback to that model.

Expect: `export_model` → download to a temp dir → edit → validate with
`write-model-file`'s validator → upload → `import_portfolio_data` → poll. The agent
should warn that re-importing a model clears omitted optional properties. Finally:

> Delete it.

Expect a refusal to act on implicit context alone: the agent should ask which
portfolio, by name.

**Record the result here** (date, server version, what broke) rather than in a
commit message.
