...
PORTFOLIO = "write-portfolio-file"
SCENARIO = "write-scenario-file"
MODEL = "write-model-file"
...
def build_runs():
    ...
    for fx in fixtures_for(MODEL):
        runs.append((MODEL, "validate_model", fx, None))
...
def check_common_copies():
    copies = [SKILLS_DIR / PORTFOLIO / "scripts" / "qp_validation_common.ps1",
              ...
              SKILLS_DIR / MODEL / "scripts" / "qp_validation_common.ps1"]
    hashes = []
    for c in copies:
        if not c.is_file():
    ...
    if len(set(hashes)) != 1:
        return ("the qp_validation_common.ps1 copies differ (must be byte-identical); "
