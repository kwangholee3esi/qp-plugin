...
PORTFOLIO_VALIDATOR = SKILLS / "write-portfolio-file" / "scripts" / "validate_portfolio.py"
SCENARIO_VALIDATOR = SKILLS / "write-scenario-file" / "scripts" / "validate_scenario.py"
MODEL_VALIDATOR = SKILLS / "write-model-file" / "scripts" / "validate_model.py"
PORTFOLIO_SCHEMA = SKILLS / "write-portfolio-file" / "schemas" / "portfolio.schema.json"
SCENARIO_SCHEMA = SKILLS / "write-scenario-file" / "schemas" / "scenario.schema.json"
MODEL_SCHEMA = SKILLS / "write-model-file" / "schemas" / "model.schema.json"
...
MODEL_FIXTURE = TESTS_DIR / "model" / "model_full_coverage.json"
...
def main():
    ...
    # 1. Validity: model fixture validates clean (self-contained; no cross-ref).
    code, result, out, err = _run([
        sys.executable, str(MODEL_VALIDATOR), str(MODEL_FIXTURE),
        "--schema", str(MODEL_SCHEMA), "--format", "json"])
    ok, detail = _is_clean(result)
    check("validate model_full_coverage.json (clean)", code == 0 and ok,
          f"exit={code} {detail} stderr={err.strip()[:200]}")
    ...
    # 2. Coverage: model schema fully covered by the model fixture.
    code, _, out, err = _run([
        sys.executable, str(COVERAGE_CHECK),
        "--schema", str(MODEL_SCHEMA), str(MODEL_FIXTURE)])
    check("coverage: model schema fully covered", code == 0,
          f"exit={code}\n{out}{err}".rstrip())
