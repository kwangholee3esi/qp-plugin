...
def walk_value(value, path, enum_paths: set, present: set, present_enum: set):
    ...
    if isinstance(value, dict):
        ...
        ip = path + "[]"
        for item in value:
            # A scalar array item can itself be an enum (e.g. metrics[].derived[]).
            if item is not None and not isinstance(item, (dict, list)) and ip in enum_paths:
                present_enum.add(f"{ip}={item}")
            walk_value(item, ip, enum_paths, present, present_enum)
