...
This area proves the three authoring skills can produce files that exercise **every
...
└── model/
    └── model_full_coverage.json       # every model field + all 22 enum values
```

The model schema's enums all live on array items (`metrics[]`, `attributes[]`,
`derived[]`), so one kitchen-sink model file covers everything.
...
Expect `8 passed, 0 failed`. Requires `jsonschema>=4.18` (Draft 2020-12), the same
dependency the validators use.

You can also run the coverage checker directly:

```
python coverage_check.py --schema ../skills/write-portfolio-file/schemas/portfolio.schema.json \
...
full ordered semantic findings. It also asserts the `qp_validation_common.ps1`
copies (one per skill) are byte-identical.
